...
def main(snapshotdate):
    ...
    # extract label store
    feature_fin_risk_df = feature_fin_risk_df.filter((col("snapshot_date") >= config["train_test_start_date"]) & (col("snapshot_date") <= config["oot_end_date"]))
    
    print("extracted features_sdf", feature_fin_risk_df.count(), config["train_test_start_date"], config["oot_end_date"])
    ...
    # extract label store
    feature_eng_df = feature_eng_df.filter((col("snapshot_date") >= config["train_test_start_date"]) & (col("snapshot_date") <= config["oot_end_date"]))
    
    print("extracted features_sdf", feature_eng_df.count(), config["train_test_start_date"], config["oot_end_date"])

    # --- prepare data for modeling ---
    # prepare data for modeling
    data_pdf = (labels_sdf
                .join(feature_fin_risk_df, on=["Customer_ID", "snapshot_date"], how="left")
                .join(feature_eng_df, on=["Customer_ID", "snapshot_date"], how="left")).toPandas()
    ...
    model_artefact['preprocessing_transformers']['stdscaler'] = pipe
